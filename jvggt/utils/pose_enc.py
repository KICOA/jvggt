...
def extri_intri_to_pose_encoding(
    ...
    Args:
        extrinsics (jt.Var): Camera extrinsic parameters with shape BxSx3x4,
            where B is batch size and S is sequence length.
            In OpenCV coordinate system (x-right, y-down, z-forward), representing camera from world transformation.
            The format is [R|t] where R is a 3x3 rotation matrix and t is a 3x1 translation vector.
        intrinsics (jt.Var): Camera intrinsic parameters with shape BxSx3x3.
    ...
    Returns:
        jt.Var: Encoded camera pose parameters with shape BxSx9.
    ...
    if pose_encoding_type == "absT_quaR_FoV":
        ...
        H, W = image_size_hw
        fov_h = 2 * jt.atan((H / 2) / intrinsics[..., 1, 1])
        fov_w = 2 * jt.atan((W / 2) / intrinsics[..., 0, 0])
        pose_encoding = jt.cat([T, quat, fov_h[..., None], fov_w[..., None]], dim=-1).float()
    else:
...
def pose_encoding_to_extri_intri(
    ...
    Args:
        pose_encoding (jt.Var): Encoded camera pose parameters with shape BxSx9,
            where B is batch size and S is sequence length.
            For "absT_quaR_FoV" type, the 9 dimensions are:
            - [:3] = absolute translation vector T (3D)
            - [3:7] = rotation as quaternion quat (4D)
    ...
    Returns:
        tuple: (extrinsics, intrinsics)
            ...
            - intrinsics (jt.Var or None): Camera intrinsic parameters with shape BxSx3x3,
    ...
    if pose_encoding_type == "absT_quaR_FoV":
        ...
        R = quat_to_mat(quat)
        extrinsics = jt.cat([R, T[..., None]], dim=-1)

        if build_intrinsics:
            H, W = image_size_hw
            fy = (H / 2.0) / jt.tan(fov_h / 2.0)
            fx = (W / 2.0) / jt.tan(fov_w / 2.0)
