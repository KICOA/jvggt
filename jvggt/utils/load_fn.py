...
def list_images_from_folder(image_folder: str) -> list[str]:
    """Sorted image paths under a folder (non-image files skipped)."""
...
def select_image_paths(image_paths: list[str], max_images: int) -> list[str]:
    """Cap views. ``max_images <= 0`` keeps all."""
...
def load_and_preprocess_images_square(image_path_list, target_size=1024):
    ...
    Returns:
        tuple: (
            jt.Var: Batched tensor of preprocessed images with shape (N, 3, target_size, target_size),
            jt.Var: Array of shape (N, 5) containing [x1, y1, x2, y2, width, height] for each image
        )
...
def load_and_preprocess_images(image_path_list, mode="crop"):
    ...
    Returns:
        jt.Var: Batched tensor of preprocessed images with shape (N, 3, H, W)
    ...
    images = jt.stack(images)  # concatenate images
