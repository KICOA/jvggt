...
class DinoVisionTransformer(nn.Module):
    ...
    def init_weights(self):
        ...
        if self.register_tokens is not None:
            normal_(self.register_tokens, std=1e-6)
    ...
    def interpolate_pos_encoding(self, x, w, h):
        ...
        return jt.cat((class_pos_embed.unsqueeze(0), patch_pos_embed), dim=1).astype(previous_dtype)

    def prepare_tokens_with_masks(self, x, masks=None):
        B, nc, w, h = x.shape
        x = self.patch_embed(x)
        if masks is not None:
            x = jt.where(masks.unsqueeze(-1), self.mask_token.astype(x.dtype).unsqueeze(0), x)

        x = jt.cat((self.cls_token.expand(x.shape[0], -1, -1), x), dim=1)
        x = x + self.interpolate_pos_encoding(x, w, h)

        if self.register_tokens is not None:
            x = jt.cat((x[:, :1], self.register_tokens.expand(x.shape[0], -1, -1), x[:, 1:]), dim=1)
    ...
    def execute(self, *args, is_training=True, **kwargs):
